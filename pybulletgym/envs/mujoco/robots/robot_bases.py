...
class XmlBasedRobot:
	...
	def addToScene(self, bullet_client, bodies):
		...
		#_ = bullet_client.loadURDF("plane.urdf", useMaximalCoordinates=True)
...
class MJCFBasedRobot(XmlBasedRobot):
	...
	def reset(self, bullet_client,joints_coef = None,dynamics_coef = None):
		...
		if self.doneLoading == 0:
			...
			if self.self_collision:
				...
				self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p, self.objects)
		self.robot_specific_reset(self._p,joints_coef=joints_coef,dynamics_coef=dynamics_coef)
